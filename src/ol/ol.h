 ...
 */
static uintptr_t new_buffer(struct olvm_t* olvm, int type, char* data, int size)
{
	int words = (size + sizeof(uintptr_t) - 1) / sizeof(uintptr_t);
	int pads = (words * sizeof(uintptr_t) - size);

	uintptr_t* p = (uintptr_t*)OLVM_allocate(olvm, words);
 ...
 */
static uintptr_t new_string(struct olvm_t* olvm, char* data)
{
 ...
 * \param size source data size (and buffer size, sure)
 */
static uintptr_t new_bytevector(struct olvm_t* olvm, char* data, int size)
{
	return new_buffer(olvm, 19, data, size);
}

uintptr_t d2ol(struct olvm_t* olvm, double v); // extern, olvm.c
static uintptr_t new_rational(struct olvm_t* olvm, double data)
{
	return d2ol(olvm, data);
}

static uintptr_t new_vptr(struct olvm_t* olvm, void* ptr)
{
	return new_buffer(olvm, 49, (char*)&ptr, sizeof(ptr));
}
